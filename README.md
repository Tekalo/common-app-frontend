...
To deploy to prod, you will need to create a full release. It will always need to have the format `${semantic version number}` eg: `1.2.0`. For specific instructions on creating a full release, please see below.
...
If you wish to run all of the tests through the command line, you can simply run `pnpm test:e2e`.
...
Unit tests have been added to the repo. They do not run as part of the CI pipeline yet, but we should probably do that. These are meant to test each component individually and set expectations for how things should behave independently. Tests should not include other providers/components if you can help it. They should not make API calls.

The tests are in a file strucutre identical to the `src` folder because most components should have a test file and this is the easiest way to keep the tests separate from the source code.
